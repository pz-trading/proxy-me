...
import ProxyForm from "./proxy_form";
...
function Proxy(){
    ...
    useEffect(()=>{
        if ( accountProfile.initialize && accountProfile.is_logged_in){
            axios.get("/be-api/get-config/", {withCredentials: true})
            .then( (res) => {
    ...
    const handleFormSave = (data, handleErrorOnSave) => {
        ...
        axios.post("/be-api/save-proxy-configuration", data).then((res) => {
            setDialogOpen(false);
    ...
    const handleFormDelete = (data, handleErrorOnSave) => {
        axios.post("/be-api/delete-proxy-configuration", data
        ).then((res) => {
            setDialogOpen(false);
            setQueryFlag(!queryFlag);
                        ...
                        <Table sx={{ minWidth: 650 }}>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell className="sCell">URL</TableCell>
                                    <TableCell className="sCell">IP</TableCell>
                                    <TableCell className="sCell">PORT</TableCell>
                                    <TableCell className="sCell">DEPARTMENT</TableCell>
                                    <TableCell className="sCell">MODE</TableCell>
                            ...
                            <TableBody>
                                {data?.map((row) => (
                                    ...
                                    <TableCell>{row.department.department}</TableCell>
